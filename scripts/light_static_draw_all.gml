...
n = ds_list_size(LS_static_lights);
draw_set_blend_mode(argument0);
for(var a = 0; a < n; a += 1) {
    l = LS_static_lights[|a];
    if(!l[?"enabled"]) {
        continue;
    }
    if(l[?"shadows"] < 2) {
        draw_sprite_ext(l[?"sprite"], 0, l[?"x"], l[?"y"], 1, 1, l[?"angle"], l[?"blend"], 1);
    } else {
        ...
        draw_set_blend_mode(argument0);
        draw_surface_ext(tl, l[?"x"] - vector_rotate_x(xo, yo, l[?"angle"]), l[?"y"] - vector_rotate_y(xo, yo, l[?"angle"]), 1, 1, l[?"angle"], l[?"blend"], 1);
    }
}
